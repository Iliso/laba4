...
public class GraphicsDisplay extends JPanel {
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            // Проверка условия
            boolean exceedsAverage = exceedsTwiceAverage(point[1]);
            canvas.setColor(exceedsAverage ? Color.GREEN : Color.RED);
...
// более чем в два раза среднее значение функции на всѐм интервале
    private boolean exceedsTwiceAverage(double value) {
        double total = 0.0;
        for (Double[] point : graphicsData) {
            total += point[1]; // Суммируем значения функции
        }
        double average = total / graphicsData.length; // Среднее значение функции
        return value > 2 * average; // Проверка условия
    }
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        double offset = 20; // сдвиг оси X вверх
        ...
        if (minY<=0.0 && maxY>=0.0) {
            ...
            // а нижняя (minY) <= 0.0
            canvas.draw(new Line2D.Double(xyToPoint(minX, offset), xyToPoint(maxX, offset)));
            // Стрелка оси X
            GeneralPath arrow = new GeneralPath();
